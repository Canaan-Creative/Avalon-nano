	...
	setInterval(function() {
		...
		if(data !== undefined){
			...
			if (data.type === P_NONCE) {
				...
				var pad = "00000000";
				nonce = pad.substring(0, pad.length - nonce.length) + nonce;
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState === 4) {
				...
				if (message.error === null) {
					if ((message.result.data !== undefined) && (message.result.midstate !== undefined)) {
...
//getWork();
//
//var pool = new Pool('stratum.btcchina.com', 3333, "canaan.apptest", "1234");
//pool.run();
