...
/* Ring buffer size */
#define BUF_CNT             4
#define UCOM_RX_BUF_SZ      (AVAM_P_COUNT * BUF_CNT)
#define UCOM_TX_BUF_SZ      (AVAM_P_COUNT * BUF_CNT)
#define UART_RX_BUF_SZ      (A3233_NONCE_SIZE * BUF_CNT)
#define UART_TX_BUF_SZ      (A3233_WORK_SIZE * BUF_CNT)
...
static ErrorCode_t hid_int_hdlr(USBD_HANDLE_T usb_h, void *data, uint32_t event)
{
	...
	case USB_EVT_IN:
		...
		if (RingBuffer_GetCount(&usb_txrb) >= AVAM_P_COUNT) {
			g_ucom.usbtxflags |= UCOM_TX_BUSY;
			RingBuffer_PopMult(&usb_txrb, g_ucom.usbtx_buff, AVAM_P_COUNT);
			USBD_API->hw->WriteEP(g_ucom.usb_h, HID_EP_IN, g_ucom.usbtx_buff, AVAM_P_COUNT);
		}
		break;
...
uint32_t hid_write(uint8_t *pbuf, uint32_t len)
{
	...
	if (g_ucom.usbtxflags & UCOM_TX_CONNECTED) {
		if (!(g_ucom.usbtxflags & UCOM_TX_BUSY) && (RingBuffer_GetCount(&usb_txrb) >= AVAM_P_COUNT)) {
			g_ucom.usbtxflags |= UCOM_TX_BUSY;
			RingBuffer_PopMult(&usb_txrb, g_ucom.usbtx_buff, AVAM_P_COUNT);
			ret = USBD_API->hw->WriteEP(g_ucom.usb_h, HID_EP_IN, g_ucom.usbtx_buff, AVAM_P_COUNT);
		}
	}
